module Members
  class PointsCounter
    ...
    def self.call(member_stats, position)
      new(member_stats, position).call
    end

    attr_accessor :member_stats, :position

    def initialize(member_stats, position)
    ...
    def clean_sheets
      case position
      when 'P'
        conceded_goals.zero? ? 1 : 0
      when 'D'
        # TODO: for defenders return 1 if their Club do not have conceded goals in this Tour
        0
      else
        0
      end
